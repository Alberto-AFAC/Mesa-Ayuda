...
function listar_tareas(){
    ...
    $("#cuadro1").slideDown("slow");
        ...
        var table = $("#tareas").DataTable({
            ...
            {"data":"ftermino"},
            {"data":"tareas"},
            {"defaultContent":"<button type='button' data-toggle='modal' data-target='#modalDetalles' class='detalle btn btn-warning'><i class='glyphicon glyphicon-list-alt'></i></button> <button type='button' data-toggle='modal' data-target='#modalAgrmast' style='color:blue;' class='agrmast btn btn-default'><i class='glyphicon glyphicon-plus'></i></button>"}
            ],
...
//console.log(res[0].id_act);
    html="<table style='width:97%; margin:0 auto;'><tr><th style='width:4%;'>N°</th><th style='width:25%;'>Tarea</th><th style='width:25%;'>Descripcion</th><th style='width:15%;'>Inicia tarea</th><th style='width:15%;'>Finaliza tarea</th><th style='width:4%;'></th><th style='width:4%;'></th><th style='width:5%;'></th></tr>";
    for(i=0;i<res.length;i++){
    var tarea = obj.data[i].id_act;
    //console.log(tarea);
    if(id==tarea){
        x++;  
        html+="<td>"+x+"</td><td>"+obj.data[i].nombre+"</td><td><textarea style='width:90%; border:1px solid transparent;'>"+obj.data[i].descripcion+"</textarea></td><td>"+obj.data[i].finicio+"</td><td>"+obj.data[i].ftermino+"</td><td>"+"<button type='button' onclick='editar("+'"'+obj.data[i].id_tareas+'"'+")' class='editar btn btn-primary' data-target='#modalEditar'><i class='fa fa-pencil-square-o'></i></button>"+"</td><td>"+"<button type='button' onclick='eliminar("+'"'+obj.data[i].id_tareas+'"'+")' class='eliminar btn btn-danger' data-toggle='modal' data-target='#modalEliminar'><i class='fa fa-trash-o'></i></button>"+"</td><td>"+"<button type='button' onclick='finalizar("+'"'+obj.data[i].id_tareas+'"'+")' class='finalizar btn btn-default' data-toggle='modal' data-target='#modalFinalizar'>Finalizar</button>"+"</td><tr>";        
        }
    }
    html+="</table>"
    $("#demo").html(html);
...
function finalizar(id_tareas){

        var tarea = id_tareas;
        var id_tareas = $("#TareaFinalizar #id_tareas").val(tarea);
   ...
   for(i=0;i<res.length;i++){
    var id_tareas = obj.data[i].id_tareas;
        ...
        if(tarea == id_tareas){             
             var id_act = obj.data[i].id_act;
             var id_activ = $("#TareaFinalizar #id_act").val(id_act);
        }
       }     
    });
...
function finalizar_tarea(){

    $("#finalizar-tarea").on("click", function(){
        var id_tareas = $("#TareaFinalizar #id_tareas").val(),
                id_act = $("#TareaFinalizar #id_act").val(),
                opcion = $("#TareaFinalizar #opcion").val();
